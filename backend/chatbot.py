...
# - ✅ FIXED: Using NEW SDK (google-genai) with Gemini 2.5 Flash
# """
...
# # ✅ CORRECT - Initialize with NEW SDK pattern
...
#     - ✅ FIXED: Using NEW SDK (google-genai) with Gemini 2.5 Flash
#     - ✅ FIXED: client.models.generate_content() pattern
#     """
...
#     async def _send_with_retry(self, user_id: str, augmented_message: str, attempt: int = 0) -> tuple[Optional[str], bool]:
#         try:
#             def _call_gemini():
#                 response = client.models.generate_content(
#                  model="gemini-2.5-flash",
#                  contents=augmented_message,
#                  timeout=45  # 45s timeout before retry
#                 )
#             return response
...
#                 timeout=45  # Wrap with asyncio timeout
#             )
#         except asyncio.TimeoutError:
#             raise TimeoutError("Gemini API did not respond within 45 seconds")
...
#         # --- Send to Gemini 2.5 Flash with retries ---
...
"""
Career Advisor Chatbot - Gemini AI + RAG
Complete rewrite with proper structure and indentation
"""
...
class CareerKnowledgeBase:
    """RAG knowledge base with TF-IDF vectorization."""
    ...
    def _load_and_index(self):
        """Load data and build TF-IDF index."""
        careers_path = DATA_DIR / "careers.json"
        courses_path = DATA_DIR / "courses.json"
        ...
        for idx, career in enumerate(self.careers):
            try:
                ...
                logger.warning(f"Skipping career {idx}: {e}")
        ...
        for idx, course in enumerate(self.courses):
            try:
                ...
                logger.warning(f"Skipping course {idx}: {e}")
        ...
        self.tfidf_matrix = self.vectorizer.fit_transform(self.documents)
        logger.info(
            f"✅ KB indexed: {len(self.careers)} careers, "
...
Your role is to:
1. Help users choose the right career path
2. Provide detailed skill gap analyses
3. Generate structured learning roadmaps
4. Recommend specific courses and certifications
5. Give placement and interview preparation guidance
...
- Be encouraging, specific, and actionable
- Include salary ranges and job market outlook when relevant
- Keep responses concise (300-500 words max unless asked for detail)
- Use emojis sparingly (🎯 📚 💡)
- Prioritize knowledge base context over general knowledge
- If no relevant context, acknowledge it and provide general guidance

IMPORTANT: You have access to retrieved context from a knowledge base. Always prioritize this context."""
    ...
    def put(self, key: str, value: Any):
        ...
        if key in self.cache:
            ...
            if len(self.cache) >= self.max_size:
                oldest_key, _ = self.cache.popitem(last=False)
                del self.access_times[oldest_key]
                logger.warning(f"LRU eviction: {oldest_key}")
...
class CareerAdvisorChatbot:
    ...
    def _build_rag_context(self, retrieved: List[Dict]) -> str:
        """Format retrieved knowledge as context."""
        if not retrieved:
            return ""
    ...
    def _extract_reply(self, response) -> str:
        ...
        try:
            ...
            logger.warning(f"Error accessing text: {e}")
        ...
        logger.error(f"Failed to extract reply. Type: {type(response)}")
    ...
    async def _send_with_retry(
        ...
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                await asyncio.sleep(wait_time)
                return await self._send_with_retry(user_id, augmented_message, attempt + 1)
            else:
                logger.error(f"Max retries exceeded: {e}", exc_info=True)
    ...
    async def chat(
        ...
        is_valid, error_msg = validate_message(message)
        if not is_valid:
            logger.warning(f"Invalid message from {user_id}: {error_msg}")
            return {
                "response": error_msg,
                "sources": [],
                "has_context": False,
        ...
        if student_profile:
            try:
                profile_context = (
                    f"\n--- STUDENT PROFILE ---\n"
                    f"Name: {student_profile.get('name', 'Student')}\n"
                    f"Education: {student_profile.get('education', 'N/A')}\n"
...
User Question: {message}"""
        ...
        # Send to Gemini
        reply, success = await self._send_with_retry(user_id, augmented_message)
